...
function Home() {
  ...
  const homeButtonAndActionButtons = (
    <>
      {isLoggedIn ? (
        ...
        >
          <Button className='max-lg:w-full'>
            <span className='flex flex-row gap-[15px] items-center'>
              <RocketIcon />
      ...
      <div className='flex gap-[60px] w-1/2 max-lg:w-auto'>
  ...
  return (
    <div className='bg-black flex flex-col items-center px-28 py-20 max-lg:px-[30px] min-h-[100vh] max-lg:min-h-[70vh] max-lg:pt-10 '>
      <div className='w-[full] max-w-[70vw] overflow-hidden'>
        <div className='flex justify-between max-lg:flex-col max-lg:pb-0  gap-[70px]'>
          <div className='flex flex-1 flex-col gap-5 w-1/2 max-lg:w-auto max-lg:mb-10 '>
          ...
          <Link
            ...
            className='rounded-lg bg-grey-dark flex flex-1 flex-col max-lg:mb-10 w-[full] h-[full]  max-w-[30vw] overflow-hidden'
          >
            <Image
            ...
            <div className='flex flex-col gap-[10px] py-[22px] px-5'>
              <p className='font-[600] text-[22px]'>{firstProject?.title}</p>
              <div className='flex gap-3'>
                <Image
                  ...
                  height={24}
                />
                <p className='text-[16px]'>{firstProject?.author.name}</p>
              </div>
            </div>
          </Link>
          {isMobile && homeButtonAndActionButtons}
        ...
        <div className='pt-[60px] '>
          <div className='flex justify-between items-center pb-20 gap-2.5 max-lg:flex-col max-lg:items-start max-lg:pb-10 '>
            ...
            <Button variant='outlined' className='max-lg:hidden'>
              <Link
                href='/top-creators'
                passHref
          ...
          <div className='flex gap-[30px] flex-wrap justify-around max-lg:mb-10'>
            {creatorsData?.getTopCreatorsForHomePage?.results.map(
              (creator, idx) => (
                <TopCreator key={creator.id} creator={creator} idx={idx + 1} />
              )
            )}
          </div>
          <Button
          ...
          <div className='flex justify-between max-lg:flex-col gap-5'>
            {PROJECT_SHELF_STEPS.map((step) => {
              ...
              return (
                <div
                  key={step.title}
                  className='flex h-[440px] w-[380px] flex-col gap-10 bg-grey-dark rounded-lg justify-center items-center p-8 flex-wrap'
                >
        ...
        <div className='bg-grey-dark p-[60px] flex justify-between rounded-lg mt-10 mr-auto ml-auto gap-20 items-center max-lg:gap-8 max-lg:items-stretch max-lg:bg-transparent max-lg:flex-col max-lg:p-0'>
          <Image
            src={newsletterImage}
